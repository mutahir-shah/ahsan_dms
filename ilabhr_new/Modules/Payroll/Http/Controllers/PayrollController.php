...
class PayrollController extends AccountBaseController
{
    ...
    public function update(Request $request, $id)
    {
        ...
        $grossEarning   = $grossEarning + $request->fixed_allowance_input;
    ...
    public function generatePaySlip(Request $request)
    {
        ...
        foreach ($users as $user) {
            ...
            if ($endDate->greaterThan($joiningDate)) {
                $payDays = $this->countAttendace($startDate, $endDate, $userId, $daysInMonth, $useAttendance, $markAbsentUnpaid, $markApprovedLeavesPaid);
                ...
                $totalBasicSalary = [];
                $employeeBasicSalary = EmployeeMonthlySalary::where('user_id', $userId)->get();

                foreach ($employeeBasicSalary as $basic) {

                    if ($basic->basic_value_type == 'fixed') {
                        $totalBasicSalary[] = $basic->basic_salary;
                    }
                    else {
                        $totalBasicSalary[] = $basic->amount / 100 * $basic->basic_salary;
                    }
                }
    ...
    public function countAttendace($startDate, $endDate, $userId, $daysInMonth, $useAttendance, $markAbsentUnpaid, $markApprovedLeavesPaid)
    {
        ...
        if ($useAttendance) {
            ...
            $holidayData = Holiday::getHolidayByDates($startDate->toDateString(), $endDate->toDateString())->pluck('holiday_date')->values(); // Getting Holiday Data
            ...
            if ($markAbsentUnpaid) {
                ...
                if ($markApprovedLeavesPaid) {
                    $presentCount = $presentCount + $absentCount + $PaidLeaveCount;
                }
                else {
    ...
    public function countFullDaysPresentByUser($startDate, $endDate, $userId, $holidayData)
    {
        ...
        $totalPresent = Attendance::select(DB::raw('count(DISTINCT DATE(attendances.clock_in_time) ) as presentCount'))
            ...
            ->where(DB::raw('DATE(attendances.clock_out_time)'), '<=', $endDate->toDateString())
            ->where('half_day', 'no')
            ->where('user_id', $userId)
