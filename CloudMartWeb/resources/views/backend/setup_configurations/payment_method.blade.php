    ...
    <div class="row">
        ...
        <div class="col-md-6">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0 h6 ">{{translate('Moyasar Credential')}}</h5>
                </div>
                <div class="card-body">
                    <form class="form-horizontal" action="{{ route('payment_method.update') }}" method="POST">
                        @csrf
                        <input type="hidden" name="payment_method" value="moyasar">
                        <div class="form-group row">
                            <input type="hidden" name="types[]" value="MOYASAR_PUBLIC_KEY">
                            <div class="col-md-4">
                                <label class="col-from-label">{{translate('MOYASAR PUBLIC KEY')}}</label>
                            </div>
                            <div class="col-md-8">
                                <input type="text" class="form-control" name="MOYASAR_PUBLIC_KEY" value="{{  env('MOYASAR_PUBLIC_KEY') }}" placeholder="{{ translate('MOYASAR PUBLIC KEY') }}" required>
                            </div>
                        </div>
                        <div class="form-group row">
                            <input type="hidden" name="types[]" value="MOYASAR_SECRET_KEY">
                            <div class="col-md-4">
                                <label class="col-from-label">{{translate('MOYASAR SECRET KEY')}}</label>
                            </div>
                            <div class="col-md-8">
                                <input type="text" class="form-control" name="MOYASAR_SECRET_KEY" value="{{  env('MOYASAR_SECRET_KEY') }}" placeholder="{{ translate('MOYASAR SECRET KEY') }}" required>
                            </div>
                        </div>
						<div class="form-group row">
                            <div class="col-md-4">
                                <label class="col-from-label">{{translate('Moyasar Sandbox Mode')}}</label>
                            </div>
                            <div class="col-md-8">
                                <label class="aiz-switch aiz-switch-success mb-0">
                                    <input value="1" name="moyasar_sandbox" type="checkbox" {{ (get_setting('moyasar_sandbox') == 1) ? 'checked' : '' }}>
                                    <span class="slider round"></span>
                                </label>
                            </div>
                        </div>
