                                        ...
                                        <ul class="list-unstyled">
                                            ...
                                            @else
                                                ...
                                                <li class="mb-2">
                                                    ...
                                                    <a href="{{ route('products.category', \App\Models\Category::find($category_id)->slug) }}" class="d-block rounded bg-white p-2 text-reset shadow-sm">
                                                        <img
                                                            src="{{ static_asset('assets/img/placeholder.jpg') }}"
                                                            data-src="{{ uploaded_asset( \App\Models\Category::find($category_id)->banner) }}"
                                                            alt="{{  \App\Models\Category::find($category_id)->getTranslation('name') }}"
                                                            class="lazyload img-fit"
                                                            height="90"
                                                            onerror="this.onerror=null;this.src='{{ static_asset('assets/img/placeholder-rect.jpg') }}';"
                                                        >
                                                        <div class="text-truncate fs-12 fw-600 mt-2 opacity-70" style="text-align: center;"><strong>{{  \App\Models\Category::find($category_id)->getTranslation('name') }}</strong></div>
                                                    </a>
                                                </li>
                                                @foreach (\App\Utility\CategoryUtility::get_immediate_children_ids($category_id) as $key => $id)
                                                    <?php
                                                        $cid = \App\Models\Category::find($id)->id;
                                                        $cnt = \App\Models\Product::where('category_id', $cid)->count();
                                                        if($cnt == "0"){
                                                            continue;
                                                        }
                                                    ?>
                                                    <li class="mb-2">
                                                        ...
                                                        <a href="{{ route('products.category',  \App\Models\Category::find($id)->slug) }}" class="d-block rounded bg-white p-2 text-reset shadow-sm">
                                                            <img
                                                                src="{{ static_asset('assets/img/placeholder.jpg') }}"
                                                                data-src="{{ uploaded_asset( \App\Models\Category::find($id)->banner) }}"
                                                                alt="{{  \App\Models\Category::find($id)->getTranslation('name') }}"
                                                                class="lazyload img-fit"
                                                                height="90"
                                                                onerror="this.onerror=null;this.src='{{ static_asset('assets/img/placeholder-rect.jpg') }}';"
                                                            >
                                                            <div class="text-truncate fs-12 fw-600 mt-2 opacity-70" style="text-align: center;"><strong>{{  \App\Models\Category::find($id)->getTranslation('name') }}</strong></div>
                                                        </a>
