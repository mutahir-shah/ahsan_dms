        ...
        <div class="row">
            ...
            <table width="100%">
                <tr class="three">
                    ...
                    {{-- @dd($order) --}}
                    <td align="left" style="width: 40%;vertical-align: top;">
                        <h3 style="margin-bottom: 5px;">{{ translateKeyword('Invoice No')}}. {{ $userRequest->booking_id }}</h3>
                        <p style="margin-bottom: 5px;">{{ translateKeyword('Date')}}: {{ $userRequest->updated_at->toFormattedDateString() }}</p>
                        <p style="margin-bottom: 5px;">
                            {{ translateKeyword('Recipient')}}: {{ $userRequest->user->first_name . ' ' . $userRequest->user->last_name  }}</p>
                    </td>
                </tr>
            </table>
            <hr/>
            <table width="100%">
                <tr class="three">
                    <td align="left" style="width: 40%;vertical-align: top;">
                        <p style="margin-bottom: 5px;"><b>{{ translateKeyword('Origin')}}:</b> {{ $userRequest->s_address }}
                            ...
                            <hr/>
                            <p style="margin-bottom: 5px;"><b>{{ translateKeyword('Destination')}}:</b> {{ $userRequest->d_address }}
            ...
            <table width="100%">
                <tr class="three">
                    <td align="left" style="width: 40%;vertical-align: top;">
                        <p>{{ translateKeyword('Service')}}: <b>{{ $userRequest->service_type->name }}</b> 
                            | {{translateKeyword('Distance')}}: <b>{{ distance($userRequest->payment->distance) }}</b> 
                            | {{ translateKeyword('Service Duration')}}: <b>{{ $userRequest->travel_time }}</b></p>
                    </td>
                </tr>
            </table>
            <hr/>
            <table width="100%">
                <tr class="three">
                    <td align="left" style="width: 100%; vertical-align: top;">
                        <p style="margin-bottom: 5px;"> @if ($userRequest->tip_amount_driver) <b> {{ translateKeyword('Thank you!')}} {{ $userRequest->user->full_name }} {{ translateKeyword('for giving a tip of')}} {{ currency($userRequest->tip_amount_driver) }} {{ translateKeyword('to')}} {{ $userRequest->provider->full_name }}</b> @endif <br/> {{ translateKeyword('We hope you enjoyed our service')}}</p>
                        <hr/>
                        <p style="margin-bottom: 5px;">{{ translateKeyword('you served by')}} <b>{{ $userRequest->provider->full_name }}</b>
            ...
            <table width="100%">
                <tr class="three">
                    <td align="left"><b>{{ translateKeyword('Charges Breakdown')}}</b></td>
                ...
                <tr class="three">
                    <td align="left"><b>{{ translateKeyword('Bank Charges Amount')}}</b></td>
                        ...
                        <td align="left" style="vertical-align: top;">
                            ...
                            @if($cardDetails)
                                <p>
                                    ...
                                    <b>{{ translateKeyword('payment')}}: {{ $cardDetails->brand }} ****{{ $cardDetails->last_four }} | {{ currency($userRequest->payment->total) }}</b></span>
                                </p>
                                @if ($userRequest->prebooking_amount !== null)
                                    <p>{{ translateKeyword('A temporary hold of')}} {{ currency($userRequest->prebooking_amount) }} {{ translateKeyword('was placed on your payment method')}} ****{{ $cardDetails->last_four }}. {{ translateKeyword('This is not a charge and will be removed')}}</p>
                                @endif
                            @endif
                        </td>
                    </tr>
                    ...
                    <tr class="three">
                        <td align="left" style="vertical-align: top;">
                            <p> 
                                ...
                                {{ translateKeyword('payment')}}: <b>{{ translateKeyword('cash')}} | {{ $userRequest->updated_at->toFormattedDateString() }} | {{ currency($userRequest->payment->total) }}</b></p>
                        </td>
                    </tr>
                </table>
            @endif
            <table width="100%">
                <tr class="three">
                    <td align="left" style="width: 100%; vertical-align: top;">
                        <p class="text-black-50">* {{ translateKeyword('charges-narration')}}</p>
                    </td>
                </tr>
            </table>
        </div>
