        ...
        <div class="row one">
            ...
            <table width="100%">
                <tr class="three">
                    ...
                    {{-- @dd($order) --}}
                    <td align="left" style="width: 40%;vertical-align: top;">
                        <h3 style="margin-bottom: 5px;">Factura n.o:
                            TK{{ substr($userRequest->provider->fleetData ? $userRequest->provider->fleetData->name : '', 0, 1) . str_pad($userRequest->provider->fleet_invoice, 4, '0', STR_PAD_LEFT) }}</h3>
                        <p style="margin-bottom: 5px;">
                            Fecha: {{ $userRequest->updated_at->toFormattedDateString() }}</p>
                        <p style="margin-bottom: 5px;">
                            Beneficiario: {{ $userRequest->user->first_name . ' ' . $userRequest->user->last_name  }}</p>
                        @if (Setting::get('fleet_manager_address_nif') == 1 && $userRequest->provider->fleetData)
                            <p style="margin-bottom: 5px;">Factura emitida
                                por: {{ $userRequest->provider->fleetData ? $userRequest->provider->fleetData->name : '' }}</p>
                            <p style="margin-bottom: 5px;">Address: {{ $userRequest->provider->fleetData->address }}</p>
                            <p style="margin-bottom: 5px;">NIF: {{ $userRequest->provider->fleetData->nif }}</p>
                        @endif
                    </td>
                </tr>
            </table>
            ...
            <table width="100%">
                <tr class="three">
                    <td align="left" style="width: 40%;vertical-align: top;">
                        <p style="margin-bottom: 5px;"><b>Origen:</b> {{ $userRequest->s_address }}
                            - {{ $userRequest->started_at->toDayDateTimeString() }}</p>
                        <p style="margin-bottom: 5px;"><b>Destino:</b> {{ $userRequest->d_address }}
            ...
            <table width="100%" class="table-bordered">
                <tr class="three">
                    ...
                    <td align="left"><b>Base imponible</b></td>
                    <td align="left"><b>IVA {{ Setting::get('tax_percentage', 10) }}%</b></td>
                    <td align="left"><b>Cantidad Total</b></td>
                </tr>
                <tr class="three">
                    {{-- <td align="left"><b>{{ $userRequest->service_type->name }}</b></td> --}}
                    <td align="left"><b>VTC</b></td>
                    ...
                    <tr class="three">
                        <td align="left" style="width: 60%; vertical-align: top;">
                            @if ($userRequest->tip_amount)
                                <p>Monto de propina: {{ currency($userRequest->tip_amount) }}</p>
                            @endif
                        </td>
                        <td align="left" style="vertical-align: top;">
                            ...
                            @php
                                $card = Card::get()->first();
                            @endphp
                            <p>Cobrado mediante tarjeta ****{{ $card->last_four }}</p>
