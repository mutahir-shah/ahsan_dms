        ...
        <div class="row">
            ...
            <table width="100%">
                <tr class="three">
                    ...
                    {{-- @dd($order) --}}
                    <td align="left" style="width: 40%;vertical-align: top;">
                        <h3 style="margin-bottom: 5px;">{{ translateKeywordMobile('Invoice No', $language_id)}}. {{ $userRequest->booking_id }}</h3>
                        <p style="margin-bottom: 5px;">{{ translateKeywordMobile('Date', $language_id)}}: {{ $userRequest->updated_at->toFormattedDateString() }}</p>
                        <p style="margin-bottom: 5px;">
                            {{ translateKeywordMobile('Recipient', $language_id)}}: {{ $userRequest->user->first_name . ' ' . $userRequest->user->last_name  }}</p>
                    </td>
                </tr>
            </table>
            <hr/>
            <table width="100%">
                <tr class="three">
                    <td align="left" style="width: 40%;vertical-align: top;">
                        <p style="margin-bottom: 5px;"><b>{{ translateKeywordMobile('Origin', $language_id)}}:</b> {{ $userRequest->s_address }}
                            | <b>{{ $userRequest->started_at->toDayDateTimeString() }}</b></p>
                            @if ($userRequest->service_type->type != 'personal_care' && $userRequest->service_type->type != 'road_assistance')    
                            <hr/>
                            <p style="margin-bottom: 5px;"><b>{{ translateKeywordMobile('Destination', $language_id)}}:</b> {{ $userRequest->d_address }}
            ...
            <table width="100%">
                <tr class="three">
                    <td align="left" style="width: 40%;vertical-align: top;">
                        <p>{{ translateKeywordMobile('Service', $language_id)}}: <b>{{ $userRequest->service_type->name }}</b> 
                            | {{translateKeywordMobile('Distance', $language_id)}}: <b>{{ distance($userRequest->payment->distance) }}</b> 
                            | {{ translateKeywordMobile('Service Duration', $language_id)}}: <b>{{ $userRequest->travel_time }}</b></p>
                    </td>
                </tr>
            </table>
            <hr/>
            <table width="100%">
                <tr class="three">
                    <td align="left" style="width: 100%; vertical-align: top;">
                        <p style="margin-bottom: 5px;"> @if ($userRequest->tip_amount_driver) <b> {{ translateKeywordMobile('Thank you!', $language_id)}} {{ $userRequest->user->full_name }} {{ translateKeywordMobile('for giving a tip of', $language_id)}} {{ currency($userRequest->tip_amount_driver) }} {{ translateKeywordMobile('to', $language_id)}} {{ $userRequest->provider->full_name }}</b> @endif <br/> {{ translateKeywordMobile('We hope you enjoyed our service', $language_id)}}</p>
                        <hr/>
                        <p style="margin-bottom: 5px;">{{ translateKeywordMobile('you served by', $language_id)}} <b>{{ $userRequest->provider->full_name }}</b>
            ...
            <table width="100%">
                <tr class="three">
                    <td align="left"><b>{{ translateKeywordMobile('Charges Breakdown', $language_id)}}</b></td>
                ...
                <tr class="three">
                    <td align="left"><b>{{ translateKeywordMobile('Bank Charges Amount', $language_id)}}</b></td>
                        ...
                        <td align="left" style="vertical-align: top;">
                            ...
                            @if($cardDetails)
                                <p>
                                    ...
                                    <b>{{ translateKeywordMobile('payment', $language_id)}}: {{ $cardDetails->brand }} ****{{ $cardDetails->last_four }} | {{ currency($userRequest->payment->total) }}</b></span>
                                </p>
                                @if ($userRequest->prebooking_amount !== null)
                                    <p>{{ translateKeywordMobile('A temporary hold of', $language_id)}} {{ currency($userRequest->prebooking_amount) }} {{ translateKeywordMobile('was placed on your payment method', $language_id)}} ****{{ $cardDetails->last_four }}. {{ translateKeywordMobile('This is not a charge and will be removed', $language_id)}}</p>
                                @endif
                            @endif
                        </td>
                    </tr>
                    ...
                    <tr class="three">
                        <td align="left" style="vertical-align: top;">
                            <p> 
                                ...
                                {{ translateKeywordMobile('payment', $language_id)}}: <b>{{ translateKeywordMobile('cash', $language_id)}} | {{ $userRequest->updated_at->toFormattedDateString() }} | {{ currency($userRequest->payment->total) }}</b></p>
                        </td>
                    </tr>
                </table>
            @endif
            <table width="100%">
                <tr class="three">
                    <td align="left" style="width: 100%; vertical-align: top;">
                        <p class="text-black-50">* {{ translateKeywordMobile('charges-narration', $language_id)}}</p>
                    </td>
                </tr>
            </table>
        </div>
