    ...
    <div class="content-wrapper">
        <div class="container-fluid">
            <div class="box box-block bg-white">
                ...
                <h5 style="margin-bottom: 2em;">{{ translateKeyword('add-zone-charge')}}</h5>
                @include('common.notify')
                <form class="form-horizontal" action="{{route('admin.zone-charges.store')}}" method="POST"
                        ...
                        <div class="col-xs-10">
                            <select name="type" class="form-control">
                                @foreach([ 'TOLL_CHARGE', 'AIRPORT_SURCHARGE', 'ADDITIONAL_CHARGE' ] as $type)
                                    <option value="{{ $type }}" @if ($type == old('type') || $loop->index == 0) selected @endif>{{ ucwords(strtolower(str_replace('_', ' ', $type))) }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-xs-10">
                            <select name="zone_id" class="form-control">
                                @foreach($zones as $zone)
                                    <option value="{{ $zone->id }}" @if ($zone->id == old('zone_id') || $loop->index == 0) selected @endif>{{ $zone->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-xs-10">
                            <select name="charge_type" class="form-control">
                                ...
                                @foreach (['FIXED' ] as $type)
                                    <option value="{{ $type }}" @if ($type == old('charge_type', 'FIXED')) selected @endif>{{ ucwords(strtolower(str_replace('_', ' ', $type))) }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-xs-10">
                            <input class="form-control" type="text" value="{{ old('charge_value') }}" name="charge_value" required
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-xs-10">
                            <button type="submit" class="btn btn-primary">{{ translateKeyword('add-zone-charge') }}</button>
