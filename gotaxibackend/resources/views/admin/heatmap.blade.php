...
@section('styles')
    ...
    <style>
        .content-wrapper {
            min-height: calc(100vh - 46px); /* Adjust for the navbar height */
            display: flex;
        ...
        <div class="container-fluid">
            <div class="box box-block bg-white border-radius-10">
                <div class="clearfix mb-1">
                    <h5 class="float-xs-left">Job Heatmap</h5>
                    ...
                    <div class="col-md-4">
                        ...
                        @if($providers->count() > 0)
                            @foreach($providers as $provider)
                                <p class="mb-0-5">{{$provider->first_name}} {{$provider->last_name}} <span
                                            class="float-xs-right">{{($provider->rating/5)*100}}%</span></p>
                                <progress class="progress progress{{$diff[array_rand($diff)]}} progress-sm"
                                          value="{{$provider->rating}}" max="5"></progress>
                            @endforeach
                        @endif
                    </div>
                </div>
