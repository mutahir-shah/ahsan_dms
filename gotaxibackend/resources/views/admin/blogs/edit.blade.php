    ...
    <div class="content-wrapper">
        <div class="container-fluid">
            <div class="box box-block bg-white">
                ...
                <h5 style="margin-bottom: 2em;">{{ translateKeyword('edit-blog') }}</h5>
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-xs-10">
                                                <input class="form-control" type="text"
                                                    value="{{ old('title_' . $language->id, $blog->translations->where('language_id', $language->id)->first()->title ?? '') }}"
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-xs-10">
                                                <textarea name="description_{{ $language->id }}" id="description_{{ $language->id }}"
                                                    {{ $index === 0 ? 'required' : '' }} placeholder="{{ $language->name }} {{ translateKeyword('Description') }}">{{ old('description_' . $language->id, $blog->translations->where('language_id', $language->id)->first()->description ?? '') }}</textarea>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
