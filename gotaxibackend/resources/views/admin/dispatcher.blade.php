...
@section('title', translateKeyword('Dispatcher'))

@section('content')
    <div class="content-wrapper" style="padding-bottom: 0 !important">
        @include('common.notify')
        <div class="alert alert-danger" id="request_already_message" style="display: none;">
            <p class="m-0"><strong>{{ translateKeyword('error') }}!</strong> {{ translateKeyword('ride_is_already_requested/scheduled') }}</p>
        </div>
        <script>
            window.__INITIAL_DATA__ = @json($data);
        </script>
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        function createRideInitialize() {
            ...
            autocomplete_source.addListener("place_changed", function (event) {
                ...
                if (place.hasOwnProperty("place_id")) {
                    if (!place.geometry) {
                        window.alert("{{ translateKeyword('autocomplete_returned_place_contains_no_geometry') }}");
                        return;
                    }
                ...
                geocoder.geocode({latLng: event.latLng}, function (results, status) {
                    if (status == google.maps.GeocoderStatus.OK) {
                        if (results[0]) {
                            ...
                            alert("{{ translateKeyword('no_address_found') }}");
                        }
                    } else {
                        alert("{{ translateKeyword('geocoder_failed_due_to') }}: " + status);
                    }
                });
        ...
        function ongoingInitialize(trip) {
            ...
            if (trip.provider) {
                ...
                provider = new google.maps.LatLng(
                    trip.provider.latitude,
                    trip.provider.longitude
                );
        ...
        function assignProviderShow(providers, trip) {
            ...
            providers.forEach(function (provider) {
                var marker = new google.maps.Marker({
                    ...
                    service_id: provider.service[0].id,
                    title: provider.first_name + " " + provider.last_name,
                    icon: provider.service[0].status == 'active' ?  "/asset/img/marker-car.png" : "/asset/img/marker-plus.png",
                });

                var content =
                    "<p>{{ translateKeyword('name') }} : " +
                    provider.first_name +
                    " " +
                    provider.last_name +
                    "</p>" +
                    "<p>{{ translateKeyword('mobile') }} : " +
                    provider.mobile +
                    "<p>{{ translateKeyword('rating') }} : " +
                    provider.rating +
                    "</p>" +
                    "<p>{{ translateKeyword('service_type') }} : " +
                    (provider.service[0]?.service_type?.name || 'N/A')+
                    "</p>" +
                    "<p>{{ translateKeyword('card_model') }}  : " +
                    (provider.service[0]?.service_model || 'N/A')+
                    "</p>" +
                    "<p>{{ translateKeyword('card_no') }}  : " +
                    (provider.service[0]?.service_number || 'N/A') +
                    "</p>" +
                    (provider.service[0].status == 'active' ? "<a href='/admin/dispatcher/trips/" +
                    trip.id +
                    "/" +
                    provider.id +
                    "' class='btn btn-success'>{{ translateKeyword('assign_this_provider') }}</a>" : "");
        ...
        function assignProviderPopPicked(serviceId, markers=mapMarkers) {
            var index;
            for (var i = markers.length - 1; i >= 0; i--) {
                if (markers[i].service_id == serviceId) {
                    index = i;
                }
        ...
        function ajaxMapData() {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    marker.addListener("click", function () {
                        assignProviderPopPicked(provider.service[0].id,googleMarkers);
                    });
        ...
        function addMarkerToMap(element, index, mapIcon=null) {
            marker = new google.maps.Marker({
                ...
                icon: mapIcon || (mapIcons[element.service ? element.service.status : element.status]),
            });
    ...
    <script type="text/javascript">
        ...
        function schedule_select() {
            if ($('#schedule').is(":checked")) {
                $("#schedule_fields").fadeIn(700);
            } else {
                $("#schedule_fields").fadeOut(700);
            }
        }
    </script>
@endsection
