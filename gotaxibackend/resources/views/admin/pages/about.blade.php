            ...
            <div class="box box-block bg-white">
                ...
                <div className="row">
                    <form action="{{ route('admin.pages.update') }}" method="POST">
                        {{ csrf_field() }}
                        <input type="hidden" name="page" value="page_about">

                        <div class="row">
                            <div class="col-xs-12">
                                <textarea name="content" id="myeditor">{{ Setting::get('page_about') }}</textarea>
                            </div>
                        </div>
                        ...
                        @if(str_contains(config('app.url'), 'https://beegone.se'))
                            ...
                            <div class="row">
                                <div class="col-xs-12">
                                    <textarea name="content_swedish" id="myeditor1">{{ Setting::get('page_about_swedish') }}</textarea>
                                </div>
                            </div>
                            <br>
                        @endif
