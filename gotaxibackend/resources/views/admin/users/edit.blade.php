    ...
    <div class="content-wrapper">
        <div class="container-fluid">
            <div class="box box-block bg-white border-radius-10">
                <a href="{{ route('admin.user.index') }}" class="btn btn-default pull-right"><i class="fa fa-angle-left"></i>
                    {{ translateKeyword('back') }}</a>

                <h5 style="margin-bottom: 2em;">{{ translateKeyword('Update_User') }}</h5>
                @include('common.notify')
                <form class="form-horizontal row" action="{{ route('admin.user.update', $user->id) }}" method="POST"
                    ...
                    @if (Setting::get('reward_point_customer', 0) == 1)
                        <div class="form-group col-6">
                            <label for="reward_points"
                                class="col-xs-12 col-form-label">{{ translateKeyword('reward-points') }}</label>
                            <div class="col-xs-12">
                                <input class="form-control" type="text" name="reward_points"
                                    oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*?)\..*/g, '$1');"
                                    value="{{ $user->reward_points }}" id="reward_points"
                                    placeholder="{{ translateKeyword('reward-points') }}">
                            </div>
                        </div>
                    @endif
                            ...
                            <div class="col-xs-12">
                                <label class="radio-inline">
                                    <input type="radio" name="gender_pref" value="female"
                                        {{ $user->gender_pref == 'female' ? 'checked' : '' }}>
                                    {{ translateKeyword('female') }}
                                </label>
                                <label class="radio-inline">
                                    <input type="radio" name="gender_pref" value="male"
                                        {{ $user->gender_pref == 'male' ? 'checked' : '' }}>
                                    {{ translateKeyword('male') }}
                                </label>
                                <label class="radio-inline">
                                    <input type="radio" name="gender_pref" value="both"
                                        {{ $user->gender_pref == 'both' ? 'checked' : '' }}>
                    ...
                    @if (Setting::get('customer_pic_mandatory', 0) == 1)
                        <div class="form-group col-12">
                            ...
                            <label for="picture"
                                class="col-xs-12 col-form-label">{{ translateKeyword('picture') }}</label>
                            <div class="col-xs-12">
                    ...
                    @if (Setting::get('address_user', 0) == 1)
                        <div class="form-group col-6">
                            ...
                            <div class="col-xs-10">
                                <input class="form-control" type="text" value="{{ old('address', $user->address) }}"
                                    name="address" required id="address"
                                    placeholder="{{ translateKeyword('address') }}">
                            </div>
                        </div>
                    @endif
                        ...
                        <div class="form-group col-6">
                            ...
                            <div class="col-xs-10">
                                <input class="form-control" type="date" value="{{ old('dob', $user->dob) }}"
                                    name="dob" required id="dob" placeholder="DOB">
                            </div>
                        </div>
                    @endif
                    @if (Setting::get('zone_restrict_module', 0) == 1 && Setting::get('zone_module', 0) == 1)
                        <div class="form-group col-6">
                            <label for="mobile"
                                class="col-xs-12 col-form-label">{{ translateKeyword('Zones') }}</label>
                            <div class="col-xs-10">
                                <select name="zone_id" class="form-control">
                                    <option value="0" selected>{{ translateKeyword('no-zone') }}</option>
                                    @foreach ($zones as $zone)
                                        <option value="{{ $zone->id }}"
                                            @if ($zone->id == $user->zone_id) selected @endif>{{ $zone->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="form-group col-12">
                            ...
                            <div class="col-xs-4">
                                <input class="form-control" type="text" name="wallet"
                                    ...
                                    oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*?)\..*/g, '$1');"
                                    id="wallet" placeholder="{{ translateKeyword('wallet') }}">
                            </div>
