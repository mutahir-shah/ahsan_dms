...
//wsdmwdkmw
    ...
    {
        $stmt = $this->con->prepare("SELECT `status` FROM `provider_services` WHERE `status` = 'active' AND `provider_id` = (SELECT `current_provider_id` FROM `user_requests` WHERE `user_requests`.`id` = ? LIMIT 1) AND `service_type_id` = (SELECT `service_type_id` FROM `user_requests` WHERE `user_requests`.`id` = ? LIMIT 1)");
        ...
        if($stmt->num_rows > 0) {
            ...
            $stmt = $this->con->prepare("UPDATE `provider_services` SET `status` = 'riding' WHERE `provider_id` = (SELECT `current_provider_id` FROM `user_requests` WHERE `user_requests`.`id` = ? LIMIT 1)");
            ...
            $stmt = $this->con->prepare("INSERT `provider_services` SET `status` = 'riding', `provider_id` = (SELECT `current_provider_id` FROM `user_requests` WHERE `user_requests`.`id` = ? LIMIT 1), `service_type_id` = (SELECT `service_type_id` FROM `user_requests` WHERE `user_requests`.`id` = ? LIMIT 1)");
    ...
    public function getconstdata()
    {
        ...
        while ($stmt->fetch()) {
            ...
            $user['key'] = $key;
            if($key == 'currency'){
                $user['value'] = $this->currencySymbol[$value];
            } else {
                $user['value'] = $value;
            }
