...
class AccountBankResource extends Controller
{
    ...
    public function approved_account(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            BankAccount::where('id', $request->id)->update(['status' => 'APPROVED']);
    ...
    public function approved_withdraw(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $bank = WithdrawalMoney::where('status', 'APPROVED')->orderBy('id', 'DESC')->get();
    ...
    public function disapproved_withdraw(Request $request)
    {
        ...
        if ($request->ajax()) {
            $withdraw = WithdrawalMoney::where('id', $request->id)->get(['bank_account_id', 'provider_id', 'amount'])->first()->toArray();
            $withdraw['status'] = null;
            WithdrawalMoney::create($withdraw);
            return WithdrawalMoney::where('id', $request->id)->update(['status' => 'DISAPPROVED']);
        } else {

            $bank = WithdrawalMoney::where('status', 'DISAPPROVED')->orderBy('id', 'DESC')->get();
    ...
    {
        $bank = WithdrawalMoney::where('status', 'WAITING')->orderBy('id', 'DESC')->get();
